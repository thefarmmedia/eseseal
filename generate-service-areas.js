...
<section class="city-hero"><div class="container"><div class="breadcrumbs"><a href="/">Home</a> / <a href="/service-areas.html">Service Areas</a> / ${c.name}</div><span class="eyebrow" style="color:#69ceff">${c.name}, Missouri · ${c.county}</span><h1>Masonry Repair & Waterproofing in ${c.name}, MO</h1><p>ESE Seal serves ${c.name} property owners with masonry restoration, tuckpointing, chimney repair, foundation crack repair, concrete repair and waterproofing. Owner Eric Enlow provides free project estimates throughout the Springfield-area service radius.</p><div class="btn-row"><a class="btn btn-green" href="/contact.html?city=${encodeURIComponent(c.name)}">Request a ${c.name} Estimate →</a><a class="btn btn-outline" href="tel:${PHONE_HREF}">Call ${PHONE}</a></div></div></section>
...
<h3 style="margin-top:34px">More ESE Seal service areas</h3><div class="nearby-links">${links.map(x=>`<a href="/service-areas/${x.slug}.html">${x.name}</a>`).join('')}<a href="/service-areas.html">View all 50 areas</a></div></div><aside class="city-cta"><span class="eyebrow">Free estimate</span><h3>Have a project in ${c.name}?</h3><p class="muted">Tell Eric what is cracked, leaking, loose or deteriorated. You will talk with the owner—not a call center.</p><a class="btn btn-green" href="/contact.html?city=${encodeURIComponent(c.name)}">Request an Estimate</a><a class="btn btn-dark" href="tel:${PHONE_HREF}">Call ${PHONE}</a><p class="muted" style="font-size:.82rem;margin-top:14px">Serving a 60-mile radius around Springfield, Missouri.</p></aside></div></section>
